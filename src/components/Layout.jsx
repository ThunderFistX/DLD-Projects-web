...
import { FiHome, FiUsers, FiFolder, FiShoppingCart, FiBarChart2, FiSettings, FiLogOut, FiMenu, FiX } from 'react-icons/fi';
  ...
  return (
   <div className="flex h-screen bg-light dark:bg-gray-900 overflow-hidden">
      ...
      <aside className={`fixed lg:static w-64 h-full bg-dark text-white flex flex-col z-30 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <nav className="flex-1 p-4 sidebar-scroll overflow-y-auto">
          <ul className="space-y-2">
            <li><NavLink to="/" className={linkClass} onClick={() => setSidebarOpen(false)}><FiHome /> Dashboard</NavLink></li>
            <li><NavLink to="/projects" className={linkClass} onClick={() => setSidebarOpen(false)}><FiFolder /> Projects</NavLink></li>
            <li><NavLink to="/users" className={linkClass} onClick={() => setSidebarOpen(false)}><FiUsers /> Users</NavLink></li>
            <li><NavLink to="/orders" className={linkClass} onClick={() => setSidebarOpen(false)}><FiShoppingCart /> Orders</NavLink></li>
            <li><NavLink to="/reports" className={linkClass} onClick={() => setSidebarOpen(false)}><FiBarChart2 /> Reports</NavLink></li>
            <li><NavLink to="/settings" className={linkClass} onClick={() => setSidebarOpen(false)}><FiSettings /> Settings</NavLink></li>
          </ul>
        </nav>
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
       ...
       <header className="bg-white dark:bg-gray-800 p-4 shadow-card flex justify-between items-center sticky top-0 z-10">
        ...
        <Modal isOpen={isProfileOpen} onClose={() => setIsProfileOpen(false)} title="Edit Profile">
          <form onSubmit={handleSaveProfile}>
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button type="button" onClick={() => setIsProfileOpen(false)} className="px-4 py-2 rounded-lg border-2 border-gray-200 hover:bg-gray-50">
                Cancel
              </button>
